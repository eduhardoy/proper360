...
const getClientes = () => {
  return dispatch => {
    ...
    return api
      .get("clientes")
      .then(cliente =>
        dispatch({ type: GET_CLIENTES_SUCCESS, payload: cliente.data.body })
      )
      .catch(err =>
        dispatch({ type: GET_CLIENTES_FAILURE, payload: err.message })
      );
  };
};
...
const postCliente = cliente => {
  return dispatch => {
    ...
    return api
      ...
      .then(cliente =>
        dispatch({ type: POST_CLIENTES_SUCCESS, payload: cliente.data.body })
      )
      .catch(err =>
        dispatch({ type: POST_CLIENTES_FAILURE, payload: err.message })
      );
  };
};
